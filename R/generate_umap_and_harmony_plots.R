generate_umap_and_harmony_plots <- function(B1_US_merge, save_name, lanes) {
 ...
 #   ggsave(paste0(save_name,"_umap.pdf"),
    png(paste0(save_name,"_umap.png"), width=1800, height=1600, res = 300)
    Seurat::DimPlot(object = B1_US_merge, reduction = "rna.umap", pt.size = .1, split.by = "BEST.GUESS")  + Seurat::NoLegend()
    dev.off()
  } else {
