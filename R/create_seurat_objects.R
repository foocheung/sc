create_seurat_objects <- function(B1_US_data, lanes) {
   ...
   for(i in 1:length(lanes)){
     B1_US_SeuratObj[[i]] <- Seurat::CreateSeuratObject(counts = (B1_US_data[[i]]$'Gene Expression'), assay = "RNA", min.feature = 0)
     B1_US_SeuratObj[[i]][["CITE"]] <- Seurat::CreateAssayObject(counts = B1_US_data[[i]]$'Antibody Capture'[1:142,])
