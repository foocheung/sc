run_cite_harmony <-function(seur_obj) {
  ...
  seur_obj <- Seurat::RunPCA(seur_obj,reduction.name = 'apca_h')
  seur_obj <- Seurat::FindNeighbors(seur_obj, "harmony",dims = 1:min(length(rownames(seur_obj[['CITE']]))-1, 20), reduction = "apca")

  seur_obj <- Seurat::FindClusters(seur_obj, graph.name = "CITE_snn", algorithm = 3, verbose = FALSE)
  seur_obj <- Seurat::RunUMAP(seur_obj, reduction = 'harmony', dims = 1:30, assay = 'ADT',
